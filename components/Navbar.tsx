  ...
  return (
    ...
    >
      ...
      <nav className="max-w-7xl mx-auto px-4 sm:px-6 flex items-center justify-between gap-4">
        {/* Hamburger (mobile) */}
        <button
          className="lg:hidden text-[#0a3428] p-1"
        ...
        <Link href="/" className="flex flex-col items-center group shrink-0">
          <span
            className="font-[var(--font-arapey)] text-[#0a3428] text-2xl md:text-3xl tracking-wide leading-none group-hover:text-[#d7b152] transition-colors duration-300"
            style={{ fontFamily: "var(--font-arapey), Georgia, serif" }}
          >
            Alis Shine
          </span>
          <span className="text-[10px] tracking-[4px] text-[#d7b152] uppercase font-semibold">
            Jewels
          </span>
        </Link>
        ...
        <ul className="hidden lg:flex items-center gap-1">
          {navLinks.map((link) => (
            ...
            >
              <Link
                href={link.href}
                className="flex items-center gap-1 px-3 py-2 text-sm text-[#121212] hover:text-[#0a3428] font-semibold tracking-wide transition-colors duration-200 relative after:absolute after:bottom-0 after:left-3 after:right-3 after:h-0.5 after:bg-[#d7b152] after:scale-x-0 hover:after:scale-x-100 after:transition-transform after:duration-300"
              >
                {link.label}
                {link.sub && <ChevronDown size={13} className="group-hover:rotate-180 transition-transform duration-200" />}
              </Link>
